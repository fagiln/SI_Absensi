...
class HomeController extends Controller
{
    ...
    public function absen_masuk_store(Request $request)
    {
        ...
        $imageName = 'absen_in_' . $userid . '_' . now()->format('YmdHi') . '.png'; // Nama file menggunakan user ID dan timestamp

        // Tentukan path penyimpanan di public/storage/kehadiran
        $destinationPath = public_path('storage/kehadiran');

        // Pastikan folder 'kehadiran' ada di dalam public/storage
        if (!file_exists($destinationPath)) {
            mkdir($destinationPath, 0755, true); // Buat folder jika belum ada
        }

        // Simpan file ke direktori public/storage/kehadiran
    ...
    public function absen_pulang_store(Request $request)
    {
        ...
        $image = str_replace('data:image/png;base64,', '', $photoData);
        $image = str_replace(' ', '+', $image);
        $imageName = 'absen_out_' . $userid . '_' . now()->format('YmdHi') . '.png'; // Nama file menggunakan user ID dan timestamp
